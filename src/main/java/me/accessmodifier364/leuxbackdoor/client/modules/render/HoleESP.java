...
import me.accessmodifier364.leuxbackdoor.client.util.Pair;
...
public class HoleESP extends Module {
	...
	Setting mode 				= create("Mode", "HoleESPMode", "Glow 2", combobox("Pretty", "Solid", "Outline", "Glow", "Glow 2"));
	Setting off_set 			= create("Height", "HoleESPOffSetSide", 1.0, 0.0, 1.0);
	Setting range   			= create("Range", "HoleESPRange", 8, 1, 12);
	...
	Setting rb 				= create("R", "HoleESPRb", 45, 0, 255);
	Setting gb 				= create("G", "HoleESPGb", 230, 0, 255);
	Setting bb 				= create("B", "HoleESPBb", 220, 0, 255);
	Setting ab				    = create("A", "HoleESPAb", 125, 0, 255);
	...
	Setting ro 				= create("R", "HoleESPRo", 205, 0, 255);
	Setting go				    = create("G", "HoleESPGo", 40, 0, 255);
	Setting bo 				= create("B", "HoleESPBo", 125, 0, 255);
	Setting ao 				= create("A", "HoleESPAo", 125, 0, 255);

	Setting line_a = create("Outline A", "HoleESPLineOutlineA", 128, 0, 255);

	Setting testr 				= create("TestRed", "TestRed", 0, 0, 255);
	Setting testg 				= create("TestGreen", "TestGreen", 0, 0, 255);
	Setting testb 				= create("TestBlue", "TestBlue", 255, 0, 255);
	Setting testa				    = create("TestAlpha", "TestAlpha", 0, 0, 255);

	Setting testr2 				= create("TestRed2", "TestRed2", 255, 0, 255);
	Setting testg2 				= create("TestGreen2", "TestGreen2", 0, 0, 255);
	Setting testb2 				= create("TestBlue2", "TestBlue2", 0, 0, 255);
	Setting testa2				    = create("TestAlpha2", "TestAlpha2", 0, 0, 255);
				...
				if (checkDual(second_pos, air_orient)) {
					...
					if (safe_sides == 8) {
						...
						if (low_ceiling_hole) {
							...
							if (!dual_hole_sides.containsKey(pos)) {
								dual_holes.add(new Pair<>(pos, false));
								dual_hole_sides.put(pos, air_orient);
							}
							if (!dual_hole_sides.containsKey(second_pos)) {
								dual_holes.add(new Pair<>(second_pos, false));
								dual_hole_sides.put(second_pos, oppositeIntOrient(air_orient));
							}
						}
					}
		...
		if (!holes.isEmpty() || !dual_holes.isEmpty()) {
			...
			for (Pair<BlockPos, Boolean> hole : holes) {
				if (hole.getValue()) {
					...
					color_r2 = testr.get_value(1);
					color_g2 = testg.get_value(1);
					color_b2 = testb.get_value(1);
					color_a2 = testa.get_value(1);
				} else if (!hole.getValue()) {
					color_r = color_r_o;
					color_g = color_g_o;
					color_b = color_b_o;
					color_a = ao.get_value(1);
					color_r2 = testr2.get_value(1);
					color_g2 = testg2.get_value(1);
					color_b2 = testb2.get_value(1);
					color_a2 = testa2.get_value(1);
				} else continue;
				...
				if (glow) {
					...
					RenderHelp.draw_gradiant_cube(RenderHelp.get_buffer_build(),
							hole.getKey().getX(), hole.getKey().getY(), hole.getKey().getZ(),
							1, off_set_h, 1,
							new Color(color_r, color_g, color_b, color_a), 
							new Color(color_r2, color_g2, color_b2, color_a2),
							"all"
					);
				...
				if (glowOutline) {
					...
					RenderHelp.draw_gradiant_outline(RenderHelp.get_buffer_build(), hole.getKey().getX(),
							...
							new Color(color_r2, color_g2, color_b2, color_a2), "all");
					RenderHelp.release();
				}
			}
	...
	public boolean isBlockHole(BlockPos block) {
		...
		return holes.contains(new Pair<>(block, true)) || holes.contains(new Pair<>(block, false));
	}
}
