...
public class KillAura extends Module {
	...
	Setting mode = create("Mode", "KillAuraMode", "Normal", combobox("32k", "Normal"));
	Setting player    = create("Player",   "KillAuraPlayer",  true);
	Setting hostile   = create("Hostile",  "KillAuraHostile", true);
	Setting sword     = create("Sword",    "KillAuraSword",   true);
	Setting sync_tps  = create("Sync TPS", "KillAuraSyncTps", true);
	Setting range     = create("Range",    "KillAuraRange",   6.0, 0.5, 6.0);
		...
		if (mc.player != null && mc.world != null) {
			...
			if (mode.in("Normal")) {
				...
				if (entity != null && start_verify) {
					// Tick.
					float tick_to_hit  = 20.0f - ModLoader.get_event_handler().get_tick_rate();
