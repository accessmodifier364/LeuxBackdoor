...
import me.accessmodifier364.leuxbackdoor.client.modules.combat.TotemPopCounter;
import me.accessmodifier364.leuxbackdoor.client.util.DamageUtil;
import me.accessmodifier364.leuxbackdoor.client.util.EnemyUtil;
...
public class NameTags extends Module {
    ...
    Setting show_totems = create("Totem Count", "NametagTotems", false);
    Setting show_invis = create("Invis", "NametagInvis", true);
    Setting reverse = create("Armour Reverse", "NametagReverse", false);
    Setting simplify = create("Simplify", "NametagSimp", true);
    Setting m_scale = create("Scale", "NametagScale", 15, 1, 15);
    Setting range = create("Range", "NametagRange", 150, 1, 250);

    Setting r = create("R", "NametagR", 120, 0, 255);
    Setting g = create("G", "NametagG", 120, 0, 255);
    Setting b = create("B", "NametagB", 240, 0, 255);
    ...
    public void render(EventRender event) {
        try {
            for (final EntityPlayer player : mc.world.playerEntities) {
                if (!(player.getHealth() == 1337)) {
                    if (!player.equals(mc.player) && player.isEntityAlive() && (!player.isInvisible() || show_invis.get_value(true)) && mc.player.getDistance(player) < range.get_value(1)) {
    ...
    private void renderNameTag(final EntityPlayer player, final double x, final double y, final double z, final float delta) {
        ...
        RenderUtil.drawRect((float)(-width - 2)-1, (float)(-(mc.fontRenderer.FONT_HEIGHT + 1))-1, width + 3f, 2.5f, red, green, blue, 0);
    ...
    private String getDisplayTag(final EntityPlayer player) {
        ...
        if (show_ping.get_value(true)) {
            try {
                ...
                if (responseTime > 300) {
                    pingStr += section_sign() + "c";
                } else if (responseTime > 150) {
                    pingStr += section_sign() + "e";
                } else {
                    pingStr += section_sign() + "a";
                }
