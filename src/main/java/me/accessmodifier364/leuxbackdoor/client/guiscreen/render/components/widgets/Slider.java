...
import me.accessmodifier364.leuxbackdoor.client.util.turok.values.TurokDouble;
...
public class Slider extends AbstractWidget {
	...
	public Slider(Frame frame, ModuleButton master, String tag, int update_postion) {
		this.frame   = frame;
		this.master  = master;
		this.setting = ModLoader.get_setting_manager().get_setting_with_tag(master.get_module(), tag);
	...
	public void render(int master_y, int separe, int absolute_x, int absolute_y) {
		...
		int ns_a = ModLoader.click_gui.theme_widget_name_b;
		...
		Draw.draw_string_shadow(this.slider_name, this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
		Draw.draw_string_shadow(slider_value, this.x + this.width - separe - font.get_string_width(slider_value) + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
	}
}