...
public class Label extends AbstractWidget {
	...
	public void render(int master_y, int separe, int absolute_x, int absolute_y) {
		...
		if (this.info) {
			Draw.draw_string_shadow(this.setting.get_value(s), this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
		} else {
			Draw.draw_string_shadow(this.label_name + " \"" + this.setting.get_value(s) + "\"", this.x + 2, this.save_y, ns_r, ns_g, ns_b, ns_a);
		}
	}
}