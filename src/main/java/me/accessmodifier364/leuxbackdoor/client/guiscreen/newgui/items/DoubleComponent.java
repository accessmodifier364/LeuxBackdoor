...
public class DoubleComponent extends Component {
    ...
    public DoubleComponent(final Setting value, final ModuleButton button, final int offset) {
        this.dragging = false;
        this.set = value;
        this.parent = button;
    ...
    public void updateComponent(final int mouseX, final int mouseY) {
        ...
        this.sliderWidth = (this.parent.parent.getWidth() - 4) * (this.set.get_value(1.0) - min) / (max - min);
        if (this.dragging) {
            if (diff == 0.0) {
                this.set.set_value(this.set.get_min(1.0));
            } else {
                final double newValue = roundToPlace((diff / this.parent.parent.getWidth()) * (max - min) + min, 2);
